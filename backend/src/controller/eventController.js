...
const addEvent = async (req, res) => {
    try {
        ...
        const newEvent = new Event({ eventName, description: eventDescription, eventDate, venue: eventVenue || "College", image: eventImage, author: eventAuthor })
...
const getAllEvents = async (req, res) => {
    try {
        const fetchedEvents = await Event.find({}).populate("author", "name profilePic allFollowers").sort({ eventDate:     1 })
