...
const RecommendedGroup = () => {
  return (
    <div className="bg-white m-10 p-4 rounded-[25px]">
      <h2 className="text-2xl font-semibold mb-4 pr-8 pl-8">Recommended Communities you</h2>
      <ul className="space-y-4">
        {groups.map((group) => (
          <li key={group.id} className="flex items-center justify-between pl-8 pr-8">
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="font-medium text-xl">{group.name}</p>
                <p className="text-xm text-[var(--colorPrimaryHover)]">{group.members} Acive</p>
              </div>
            </div>
        ...
        <Link to="/community" className="mt-5 text-[var(--colorPrimary)] pr-8 pl-8 text-xl">See all</Link>
      </ul>
    </div>
  );
};
